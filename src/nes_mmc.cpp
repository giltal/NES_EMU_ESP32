...
void mmc_bankvrom(int size, uint32 address, int bank)
{
   ...
   default:
      LOG_PRINT("invalid VROM bank size %d\n", size);
   }
}
...
void mmc_bankrom(int size, uint32 address, int bank)
{
   ...
   default:
      LOG_PRINT("invalid ROM bank size %d\n", size);
      break;
   }
...
mmc_t *mmc_create(rominfo_t *rominfo)
{
   ...
   LOG_PRINT("created memory mapper: %s\n", (*map_ptr)->name);
