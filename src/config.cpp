...
static myvar_t *my_create(const char *group, const char *key, const char *value)
{
   ...
   if ((var->group = (char *)malloc(strlen(group) + 1))
...
static char *my_getline(FILE *stream)
{
   ...
   do
   {
      ...
      if (NULL == dynamic)
      {
         dynamic = (char *)malloc(strlen(buf) + 1);
         if (NULL == dynamic)
         {
            return 0;
         }
      ...
      else
      {
         ...
         char *temp = NULL;
         temp = (char *)malloc(strlen(dynamic) + strlen(buf) + 1);
         if (NULL == temp)
            return 0;
      ...
      while ((line = my_getline(config_file)))
      {
         ...
         do 
         {
            ...
            case '[':
               ...
               if (NULL == s)
               {
                  LOG_PRINT("load_config: missing ']' after group\n");
               ...
               if ((value = (char *)malloc(strlen(group) + 1)))
               {
                  strcpy(value, group);
               }
               group = value;
            ...
            default:
               ...
               if (NULL == s)
               {
                  LOG_PRINT("load_config: missing '=' after key\n");
                  s = key + strlen(key);
               }
               else
               {
...

               {
                  ...
                  if (NULL == var)
                  {
                     LOG_PRINT("load_config: my_create failed\n");
                     return -1;
                  }
