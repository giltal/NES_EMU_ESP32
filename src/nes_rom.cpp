...
void rom_savesram(rominfo_t *rominfo)
{
   ...
   {
      LOG_PRINT("Preparing to save sram file for rom %s\n", rominfo->filename);
      ...
      if (NULL != fp)
      {
         ...
         fclose(fp);
         LOG_PRINT("Wrote battery RAM to %s.\n", fn);
      } else {
         LOG_PRINT("Write file handle was null for %s.\n", fn);
      }
   }
}
...
static void rom_loadsram(rominfo_t *rominfo)
{
   ...
   if (rominfo->flags & ROM_FLAG_BATTERY)
   {
      LOG_PRINT("Preparing to load sram file for rom %s\n", rominfo->filename);
      strncpy(fn, rominfo->filename, PATH_MAX);
      osd_newextension(fn, ".sav");
      ...
      if (NULL != fp)
      {
         ...
         fclose(fp);
         LOG_PRINT("Read battery RAM from %s.\n", fn);
      }
   }
}
...
{
   /* Load up SRAM */
   rominfo->sram = (uint8*)malloc(SRAM_BANK_LENGTH * rominfo->sram_banks);
...
//      fread(rominfo->sram + TRAINER_OFFSET, TRAINER_LENGTH, 1, fp);
      ...
      rom+=TRAINER_LENGTH;
      LOG_PRINT("Read in trainer at $7000\n");
   }
}
...
*/
   ...
   {
      rominfo->vram = (uint8*)malloc(VRAM_LENGTH);
...
static void rom_checkforpal(rominfo_t *rominfo)
{
   ...
   LOG_PRINT("Game specific palette found -- assuming VS. UniSystem\n");
}
...
//   _fread(&head, 1, sizeof(head), fp);
   ...
   else
   {
      ...
      if (('D' == head.mapper_hinybble) && (0 == memcmp(head.reserved, "iskDude!", 8)))
         LOG_PRINT("`DiskDude!' found in ROM header, ignoring high mapper nybble\n");
      else
      {
         LOG_PRINT("ROM header dirty, possible problem\n");
         rominfo->mapper_number |= (head.mapper_hinybble & 0xF0);
      }
...
char *rom_getinfo(rominfo_t *rominfo)
{
   ...
   LOG_PRINT(temp, " [%d] %dk/%dk %c", rominfo->mapper_number,
...
void rom_free(rominfo_t **rominfo)
{
   ...
   if ((*rominfo)->flags & ROM_FLAG_VERSUS)
   {
      ...
      LOG_PRINT("Default NES palette restored\n");
   }
